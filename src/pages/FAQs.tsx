    ...
    <div className="min-h-screen bg-brand-dark text-brand-text hero-background">
      ...
      <main className="relative z-10 py-20 px-6 md:px-12 lg:px-24 max-w-7xl mx-auto">
        <h1 className="text-5xl md:text-6xl font-extrabold text-center leading-tight mb-8">
          Frequently Asked <span className="bg-gradient-to-r from-brand-orange to-brand-light-orange text-transparent bg-clip-text">Questions</span>
        </h1>
          ...
          <Accordion type="single" collapsible className="w-full">
            {faqs.map((faq, index) => (
              <AccordionItem key={index} value={`item-${index}`} className="border-b border-brand-border">
                <AccordionTrigger className="text-lg font-semibold text-brand-text hover:no-underline hover:text-brand-light-orange transition-colors py-4">
                  {faq.question}
                </AccordionTrigger>
                <AccordionContent className="text-brand-text-muted text-base pb-4">
                  {faq.answer}
                </AccordionContent>
              </AccordionItem>
            ))}
