    ...
    <div className="min-h-screen bg-brand-dark text-brand-text hero-background">
      ...
      <main className="relative z-10 py-20 px-6 md:px-12 lg:px-24 max-w-7xl mx-auto">
        <h1 className="text-5xl md:text-6xl font-extrabold text-center leading-tight mb-8">
          Our <span className="bg-gradient-to-r from-brand-orange to-brand-light-orange text-transparent bg-clip-text">Blog</span>
        </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {blogPosts.map((post) => (
            <Card key={post.id} className="bg-gradient-dark-card border border-brand-border rounded-xl shadow-lg overflow-hidden">
              <img src={post.imageUrl} alt={post.title} className="w-full h-48 object-cover" />
              <div className="p-6">
                <h3 className="text-2xl font-bold text-brand-text mb-3">{post.title}</h3>
                <div className="flex items-center text-brand-text-muted text-sm mb-4 space-x-4">
                ...
                <p className="text-brand-text-muted mb-6">{post.excerpt}</p>
                <Link to={`/blog/${post.id}`} className="text-brand-orange hover:text-brand-light-orange font-semibold transition-colors">
                  Read More &rarr;
                </Link>
              </div>
            </Card>
