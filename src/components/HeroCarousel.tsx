...
const CarouselSlide: React.FC<CarouselSlideProps> = ({ icon: Icon, title, description, iconColor }) => (
  <Card className="w-full max-w-md bg-gradient-dark-card border border-qicky-blue/30 rounded-xl p-6 shadow-lg flex-shrink-0">
  ...
  return (
    <div className="relative w-full max-w-md">
      ...
      <div className="absolute bottom-4 left-0 right-0 flex items-center justify-between px-4">
        ...
        <div className="flex space-x-2">
          <Button variant="ghost" size="icon" className="text-qicky-textmuted hover:bg-qicky-blue/20 rounded-full" onClick={scrollPrev}>
            <ArrowRight className="h-5 w-5 rotate-180" />
          </Button>
          <Button variant="ghost" size="icon" className="text-qicky-textmuted hover:bg-qicky-blue/20 rounded-full" onClick={scrollNext}>
